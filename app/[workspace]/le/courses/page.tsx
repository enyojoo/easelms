...
export default function CoursesPage({ params }: { params: { workspace: string } }) {
  ...
  const completedCourses = courses.filter((course) => user.completedCourses.includes(course.id) || course.id === 4)
    ...
    const getCTAButton = () => {
      const viewCourseButton = (
        <Button variant="outline" asChild className="flex-1">
          <Link href={`/${params.workspace}/courses/${course.id}`}>View Course</Link>
        </Button>
      )
      ...
      if (isCompleted) {
        return (
          <>
            {viewCourseButton}
            <Button variant="outline" asChild className="flex-1">
              <Link href={`/${params.workspace}/courses/${course.id}/learn/summary`}>View Summary</Link>
            </Button>
          </>
        )
      } else if (isEnrolled) {
        return (
          <>
            ...
            <Button asChild className="flex-1">
              <Link href={`/${params.workspace}/courses/${course.id}`}>
          ...
          case "free":
            return (
              <>
                {viewCourseButton}
                <Button asChild className="flex-1">
                  <Link href={`/${params.workspace}/courses/${course.id}`}>Start Free Course</Link>
                </Button>
              </>
            )
          case "buy":
            return (
              <>
                {viewCourseButton}
                <Button asChild className="flex-1">
                  <Link href={`/${params.workspace}/courses/${course.id}`}>Buy for $49</Link>
                </Button>
              </>
            )
          case "subscribe":
            return (
              <>
                {viewCourseButton}
                <Button asChild className="flex-1">
                  <Link href={`/${params.workspace}/courses/${course.id}`}>Subscribe for $20/mo</Link>
                </Button>
              </>
            )
          case "request":
            return (
              <>
                {viewCourseButton}
                <Button asChild className="flex-1">
                  <Link href={`/${params.workspace}/courses/${course.id}`}>Request Access</Link>
                </Button>
              </>
            )
        }
