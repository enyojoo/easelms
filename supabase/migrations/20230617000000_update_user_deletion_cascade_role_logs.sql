-- Drop the existing foreign key constraint on role_change_logs
ALTER TABLE IF EXISTS public.role_change_logs
DROP CONSTRAINT IF EXISTS role_change_logs_user_id_fkey;

-- Recreate the foreign key constraint with ON DELETE CASCADE
ALTER TABLE IF EXISTS public.role_change_logs
ADD CONSTRAINT role_change_logs_user_id_fkey
FOREIGN KEY (user_id)
REFERENCES auth.users(id)
ON DELETE CASCADE;

-- Drop the existing foreign key constraint on public.users
...
BEGIN
    ...
    -- Delete related role change logs
    DELETE FROM public.role_change_logs WHERE user_id = OLD.id;
    
    RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create a trigger for user deletion if it doesn't exist
